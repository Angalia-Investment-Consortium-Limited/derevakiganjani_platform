...
export const useElimika = () => {
  ...
  /**
   * Fetch Driver Profile by user email
   * This is needed because AuthContext returns user email, not Driver Profile document name
   */
  const useDriverProfileByUser = (userEmail: string | undefined) => {
    ...
    return useFrappeGetDocList<any>('Driver Profile', {
      fields: ['name', 'full_name', 'user', 'phone_number'],
      filters,
      limit: 1
