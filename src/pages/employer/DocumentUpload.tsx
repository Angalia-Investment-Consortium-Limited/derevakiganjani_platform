...
import { uploadFile, saveDocument, getEmployerDocuments, submitVerification } from '@/lib/firebase';
...
const DocumentUpload: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      loadExistingDocuments(currentUser.uid);
    }
  }, [currentUser]);

  const loadExistingDocuments = async (employerId: string) => {
    try {
      const existingDocs = await getEmployerDocuments(employerId) as EmployerDocument[];
  ...
  const uploadDocument = async (documentType: string) => {
    ...
    if (!doc || !doc.file || !currentUser) return;

    setUploading(documentType);
    setError(null);

    try {
      const filePath = `employer-documents/${currentUser.uid}/${doc.file.name}`;
      const fileURL = await uploadFile(doc.file, filePath);
      await saveDocument(currentUser.uid, documentType, fileURL);

      setDocuments(prev => prev.map(d =>
        d.type === documentType
          ? { ...d, uploaded: true, status: 'uploaded' }
          : d
      ));
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      setError(null);
      await submitVerification(currentUser.uid);
