        ...
        <div className="max-w-5xl mx-auto">
                ...
                <Card key={service.id} className="hover:shadow-xl transition-shadow duration-300">
                  ...
                  <CardContent>
                    <Button onClick={() => navigate(service.path)} className="w-full">
                      {t('start_now')} <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </CardContent>
                </Card>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <section className="lg:col-span-2">
                <h2 className="text-2xl font-bold tracking-tight mb-4">{t('recent_applications')}</h2>
                <Card>
                                ...
                                <div key={app.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            {t('submitted_on')}: {app.submittedOn ? format(app.submittedOn.toDate(), 'PPP') : 'N/A'}
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <Button variant="outline" size="sm" asChild>
                                            <Link to={`/license/application/${app.id}`}>{t('view_details')}</Link>
                                        </Button>
                                    </div>
                                </div>
                            ))}
                    ...
                    {recentApplications.length > 0 && (
                        <CardHeader className="border-t">
                             <Button variant="secondary" className="w-full" asChild>
                                <Link to="/license/my-applications">{t('view_all_applications')}</Link>
                            </Button>
                        </CardHeader>
                    )}
                </Card>
            ...
            <aside>
              <h2 className="text-2xl font-bold tracking-tight mb-4">{t('quick_actions')}</h2>
