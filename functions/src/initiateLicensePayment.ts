...
interface RequestData {
    ...
    phone: string;
    application: LicenseApplicationData;
}

export const initiateLicensePayment = onCall({ 
    cpu: 1, 
    memory: '256MiB', 
    region: 'us-central1', 
    ...
    logger.info("--- initiateLicensePayment: Start ---");
    ...
    const { applicationId, phone, application } = request.data as RequestData;
    if (!applicationId || !phone || !application) {
        logger.error("Data validation failed: Missing required fields.", { applicationId, phone, application });
    ...
    let buyerName = userEmail?.split('@')[0] || "Dereva User";
    try {
    ...
    try {
        ...
        // Update the license application with the paymentId
        await admin.firestore().collection("license_applications").doc(applicationId).update({
            paymentId: paymentDocRef.id,
            status: 'pending-payment'
        });

        logger.info(`Created pending payment document ${paymentDocRef.id} for application ${applicationId}`);
        ...
        const orderJson = {
            ...
            currency: "TZS",
            no_of_items: 1,
            buyer_remarks: `Payment for Leseni: ${application.type}`,
            merchant_remarks: "Leseni Application Payment",
        ...
        // Also update the license application status to reflect the failure
        await admin.firestore().collection("license_applications").doc(applicationId).update({
